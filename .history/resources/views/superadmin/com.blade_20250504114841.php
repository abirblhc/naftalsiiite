            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div class="col-span-1 md:col-span-2 lg:col-span-3">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        @if($commercialBranche && $commercialBranche->children)
                            @foreach($commercialBranche->children as $child)
                                @if($child->name === 'Agence')
                                    <!-- Agence Card -->
                                    <a href="{{ route('sites.branche', ['site' => $site->id, 'branchType' => 'Agence']) }}">
                                                ...
                                                <div class="flex flex-wrap gap-2">
                                                    @if($child->children)
                                                        @foreach($child->children as $subChild)
                                                            @if($subChild->name !== 'GD')
                                                                <a href="{{ route('sites.branche.detail', ['site' => $site->id, 'branchType' => 'Agence', 'branch' => $subChild->id]) }}" class="flex-1 min-w-[120px]">
                                ...
                                @elseif($child->name === 'LP')
                                    <!-- LP Card -->
                                    <a href="{{ route('sites.branche', ['site' => $site->id, 'branchType' => 'LP']) }}">
                                        <div class="bg-white rounded-xl shadow-lg overflow-hidden h-full">
                                            ...
                                            <div class="p-4">
                                                <div class="flex flex-wrap gap-2">
                                                    @if($child->children)
                                                        @foreach($child->children as $subChild)
                                                            <a href="{{ route('sites.branche.detail', ['site' => $site->id, 'branchType' => 'LP', 'branch' => $subChild->id]) }}" class="flex-1 min-w-[120px]">
                                ...
                                @elseif($child->name === 'CDD')
                                    <!-- CDD Card -->
                                    <a href="{{ route('sites.branche', ['site' => $site->id, 'branchType' => 'CDD']) }}">
                                        <div class="bg-white rounded-xl shadow-lg overflow-hidden h-full">
                                            ...
                                            <div class="p-4">
                                                <div class="flex flex-wrap gap-2">
                                                    @if($child->children)
                                                        @foreach($child->children as $subChild)
                                                            <a href="{{ route('sites.branche.detail', ['site' => $site->id, 'branchType' => 'CDD', 'branch' => $subChild->id]) }}" class="flex-1 min-w-[120px]">
                        ...
                        @else
                            <div class="col-span-3 text-center py-8">
                                <p class="text-gray-500">Aucune structure commerciale trouvée.</p>
                            </div>
                        @endif
                    </div>
                </div>
                                        ...
                                        <div x-show="isOpen" x-transition class="p-4 bg-purple-50">
                                            <div class="flex flex-col gap-2">
                                                @foreach($subChild->children as $station)
                                                    <a href="{{ route('sites.branche.detail', ['site' => $site->id, 'branchType' => 'GD', 'branch' => $station->id]) }}" class="block">
