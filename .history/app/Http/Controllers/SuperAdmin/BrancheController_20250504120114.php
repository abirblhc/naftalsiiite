...
public function commercialStructure()
{
    try {
        ...
        foreach ($commercialBranches as $branch) {
            Log::info('Commercial Branche Children for site ' . $branch->site->name . ':', [
                'children' => $branch->children->pluck('name')->toArray()
            ]);
        }
...
public function showBranch($site, $branchType)
{
    try {
        $site = Site::findOrFail($site);
        $branch = Branche::where('site_id', $site->id)
            ->where('name', $branchType)
            ->firstOrFail();

        return view('superadmin.sitess', [
            'site' => $site,
            'branch' => $branch,
            'branchType' => $branchType
        ]);
    } catch (\Exception $e) {
        Log::error('Error in showBranch: ' . $e->getMessage());
        return redirect()->route('superadmin.com')->with('error', 'Branche non trouvée');
    }
}

public function showBranchDetail($site, $branchType, $branch)
{
    try {
        $site = Site::findOrFail($site);
        $branch = Branche::findOrFail($branch);
        ...
        Log::error('Error in showBranchDetail: ' . $e->getMessage());
        return redirect()->route('superadmin.com')->with('error', 'Branche non trouvée');
    }
}
