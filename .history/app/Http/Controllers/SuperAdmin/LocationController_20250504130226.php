...
public function storeMaterial(Request $request, $locationId, $entityType, $entityId)
{
    ...
    try {
        ...
        // Set the appropriate relationship based on entity type
        if ($entityType === 'room') {
        ...
        // Associate with the specific material type if it exists
        if ($materialable) {
        ...
        // Associate with the entity (room or corridor)
        if ($entityType === 'room') {
            $entity->materials()->save($material);
        } else {
            $entity->materials()->save($material);
        }
        ...
        return back()->with('error', 'Erreur lors de l\'ajout du matériel: ' . $e->getMessage());
    }
}
