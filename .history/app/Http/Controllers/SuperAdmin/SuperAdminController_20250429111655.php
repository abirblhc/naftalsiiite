...
class SuperAdminController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            'branche_name' => 'required|in:Commercial,Carburant', // Validation par nom exact
        ]);

        $branche = Branche::where('name', $request->branche_name)->first();

        if (!$branche) {
            return back()->withErrors(['branche_name' => 'La branche sélectionnée n\'existe pas.'])->withInput();
        }
