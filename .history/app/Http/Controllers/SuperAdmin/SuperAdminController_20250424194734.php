...
class SuperAdminController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Préparer l'email en minuscules
        ...
        if (!$user) {
            return redirect()->back()->with('error', 'Erreur lors de la création de l\'utilisateur. Veuillez réessayer.');
        }

        // Test de connexion avec les identifiants
        if (Auth::attempt(['email' => $email, 'password' => $defaultPassword])) {
            Auth::logout(); // Se déconnecter après le test
        }

        // Redirect back with detailed success message
    ...
    public function showCbr()
    {
        ...
        $carburantBranches = Branche::with('site')->where('name', 'carburant')->get();
...
public function destroyReclamation($id)
{
    ...
    if ($reclamation) {
        $reclamation->delete();
        return redirect()->route('superadmin.reclamations.index')
